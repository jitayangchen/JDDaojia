...
//        values.put(GoodsOrderContract.GOODS_ARRAY, goodsOrderBean.getGoodsArray());
    ...
    public boolean updateOrderEvaluation(String orderNumber, int orderEvaluation) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(GoodsOrderContract.ORDER_EVALUATION, orderEvaluation);
        int update = db.update(GoodsOrderContract.TABLE_NAME, values, GoodsOrderContract.ORDER_NUMBER + "=?", new String[]{orderNumber});
        return update >= 0;
    }
...
//            goodsOrderBean.setGoodsArray(cursor.getString(cursor.getColumnIndex(GoodsOrderContract.GOODS_ARRAY)));
    ...
    public GoodsOrderBean queryOrderByOrderNumber(String orderNumber) {
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        Cursor cursor = db.query(GoodsOrderContract.TABLE_NAME, null, GoodsOrderContract.ORDER_NUMBER + "=?", new String[]{orderNumber}, null, null, GoodsOrderContract._ID + " DESC");
...
//            goodsOrderBean.setGoodsArray(cursor.getString(cursor.getColumnIndex(GoodsOrderContract.GOODS_ARRAY)));
    ...
    public List<GoodsOrderBean> queryEvaluationOrder() {
        List<GoodsOrderBean> goodsOrderBeans = new ArrayList<>();
        SQLiteDatabase db = dbHelper.getReadableDatabase();
